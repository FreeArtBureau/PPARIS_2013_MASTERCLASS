...
//            n2 = normals[i][j+1].add( normals[i+1][j+1] ).add( points[i+1][j] );
              ...
              n1 = ( points[i][j+1].sub(points[i][j]) ).cross( points[i+1][j].sub(points[i][j]) );
              n2 = ( points[i+1][j+1].sub(points[i][j+1]) ).cross( points[i+1][j].sub(points[i+1][j+1]) );
            
            n1 = n1.normalize();
            n2 = n2.normalize();
            
            
            mesh.addFace( points[i][j], points[i][j+1], points[i+1][j], n1);
            mesh.addFace( points[i][j+1], points[i+1][j+1], points[i+1][j], n2 );
